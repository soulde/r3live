...
ImuProcess::ImuProcess(rclcpp::Node::SharedPtr nh_) : nh(nh_), b_first_frame_(true), imu_need_init_(true),
                                                       last_imu_(nullptr), start_timestamp_(-1) {
...
void ImuProcess::Reset() {
    RCLCPP_WARN(nh->get_logger(), "Reset ImuProcess");
    angvel_last = Zero3d;
    cov_proc_noise = Eigen::Matrix<double, DIM_OF_PROC_N, 1>::Zero();
...
void ImuProcess::IMU_Initial(const MeasureGroup &meas, StatesGroup &state_inout, int &N) {
    ...
    RCLCPP_INFO(nh->get_logger(), "IMU Initializing: %.1f %%", double( N ) / MAX_INI_COUNT * 100);
...
bool check_state(StatesGroup &state_inout) {
    ...
    for (int idx = 0; idx < 3; idx++) {
        if (fabs(state_inout.vel_end(idx)) > 10) {
            ...
            for (int i = 0; i < 10; i++) {
                cout << __FILE__ << ", " << __LINE__ << ", check_state fail !!!! " << state_inout.vel_end.transpose()
                     << endl;
            }
...
StatesGroup
ImuProcess::imu_preintegration(const StatesGroup &state_in, std::deque<sensor_msgs::msg::Imu::ConstSharedPtr> &v_imu,
                               double end_pose_dt) {
    ...
    for (std::deque<sensor_msgs::msg::Imu::ConstSharedPtr>::iterator it_imu = v_imu.begin();
        ...
        angvel_avr << 0.5 * (head->angular_velocity.x + tail->angular_velocity.x), 0.5 * (head->angular_velocity.y +
                                                                                          tail->angular_velocity.y),
                0.5 * (head->angular_velocity.z + tail->angular_velocity.z);
        acc_avr << 0.5 * (head->linear_acceleration.x + tail->linear_acceleration.x),
                0.5 * (head->linear_acceleration.y + tail->linear_acceleration.y), 0.5 * (head->linear_acceleration.z +
        ...
        if (rclcpp::Time(tail->header.stamp).seconds() < state_inout.last_update_time) {
            continue;
        }

        if (if_first_imu) {
            if_first_imu = 0;
            dt = rclcpp::Time(tail->header.stamp).seconds() - state_inout.last_update_time;
        } else {
        ...
        cov_w.block<3, 3>(9, 9).diagonal() =
                Eigen::Vector3d(COV_BIAS_GYRO_NOISE_DIAG, COV_BIAS_GYRO_NOISE_DIAG, COV_BIAS_GYRO_NOISE_DIAG) * dt *
                dt; // bias gyro covariance
        cov_w.block<3, 3>(12, 12).diagonal() =
                Eigen::Vector3d(COV_BIAS_ACC_NOISE_DIAG, COV_BIAS_ACC_NOISE_DIAG, COV_BIAS_ACC_NOISE_DIAG) * dt *
                dt; // bias acc covariance
    ...
    state_inout.last_update_time = rclcpp::Time(v_imu.back()->header.stamp).seconds() + dt;
...
void ImuProcess::lic_point_cloud_undistort(const MeasureGroup &meas, const StatesGroup &_state_inout,
    ...
    for (auto it_kp = IMU_pose.end() - 1; it_kp != IMU_pose.begin(); it_kp--) {
        ...
        for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--) {
            ...
            Eigen::Vector3d T_ei(
                    pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt + R_i * Lidar_offset_to_IMU - pos_liD_e);
    ...
    if (imu_need_init_) {
        ...
        if (init_iter_num > MAX_INI_COUNT) {
            ...
            RCLCPP_INFO(nh->get_logger(),
                        ...
                        stat.gravity[0], stat.gravity[1], stat.gravity[2], stat.bias_g[0], stat.bias_g[1],
                        stat.bias_g[2], cov_acc[0],
